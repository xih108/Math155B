...
public:
	...
	const VectorR3& GetScaledInvCentralAxis( ) const { return AxisC; }
	const VectorR3& GetScaledInvAxisA( ) const { return AxisA; }
	...
	VectorR3 GetCentralAxis( ) const { return AxisC*RadiusC; }
	VectorR3 GetAxisA( ) const { return AxisA*RadiusA; }
	VectorR3 GetAxisB( ) const { return AxisB*RadiusB; }
	void GetAxes( double* axes ) const;
	void GetAxes( float* axes ) const;

    // These return the axes scaled by the radius.  Divide by the radius to get the unit axes.
    const VectorR3 GetScaledCentralAxis() const { return AxisC * (RadiusC*RadiusC); }
    const VectorR3 GetScaledAxisA() const { return AxisA * (RadiusA*RadiusA); }
    const VectorR3 GetScaledAxisB() const { return AxisB * (RadiusB*RadiusB); }

    double GetRadiusA () const { return RadiusA; }
	double GetRadiusB () const { return RadiusB; }
	double GetRadiusC () const { return RadiusC; }	// Radius C is contral axis radius
