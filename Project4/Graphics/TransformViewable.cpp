/*
 *
 * RayTrace Software Package, release 3.1.2  June 8, 2017
 *
...
{
	BezierArray& thePatchesOrig = theObject->GetPatchList();
    long numPatches = thePatchesOrig.SizeUsed();
    for (long i = 0; i<numPatches; i++) {
        BezierPatch* thisBezierPatch = &(thePatchesOrig[i]);
        TransformBezierPatchRecursive(theTransform, thisBezierPatch);
    }

    BezierArray& thePatchesInternal = theObject->GetInternalPatchList();
    numPatches = thePatchesInternal.SizeUsed();
    for (long i = 0; i<numPatches; i++) {
        BezierPatch* thisBezierPatch = &(thePatchesInternal[i]);
        TransformBezierPatchRecursive(theTransform, thisBezierPatch);
    }
...
void TransformWithRigid(  ViewableEllipsoid* theObject, const RigidMapR3& theTransform )
{
	...
	theTransform.Transform(theObject->GetCenter(), &newCenter);
	// Transform the axes
	theTransform.Transform3x3(theObject->GetCentralAxis(), &newAxC);
	theTransform.Transform3x3(theObject->GetAxisA(), &newAxA);
