...
CameraView& CameraView::SetFromAABB(const AABB& boundingbox, double fovy)
{
    assert(boundingbox.WellFormed() && !boundingbox.IsFlatY() && !boundingbox.IsFlatX() && fovy>0.0);
    const VectorR3& boxmin = boundingbox.GetBoxMin();
    const VectorR3& boxmax = boundingbox.GetBoxMax();
    ScreenHeight = boxmax.y - boxmin.y;
    ScreenWidth = boxmax.x - boxmin.x;
    double halfz = 0.5*(boxmax.z - boxmin.z);
    double tanV = tan(0.5*fovy);
    ScreenDistance = halfz + ((0.5*ScreenHeight) / tanV);
    double extraFactor = 1.0 + (tanV*halfz) / ScreenHeight;
    ScreenHeight *= extraFactor;
    ScreenWidth *= extraFactor;
    ScreenCenter = 0.5*(boxmax + boxmin);
    Position.Set(ScreenCenter.x, ScreenCenter.y, ScreenCenter.z + ScreenDistance);
    View::SetDirection(0.0, 0.0, -1.0);
    DistanceHasBeenSet = true;
    PixelDirPreCalc();
    return *this;
}
...
void CameraView::RecalcPixeldUdV()
{
    ...
    assert(pixeldU.NormSq() > 0.0 && pixeldV.NormSq());
}
...
// Given a fully established camera and view, move the viewer and screen together
//   Shifts viewer and center of view 
//   Operates in local coordinates!
void CameraView::MoveCameraView(const VectorR3& displacement)
{
    VectorR3 globalDisp = displacement.z*Direction;
    globalDisp.AddScaled(pixeldU, displacement.x / pixeldU.Norm());
    globalDisp.AddScaled(pixeldV, displacement.y / pixeldV.Norm());

    Position += globalDisp;
    ScreenCenter += globalDisp;
}

void CameraView::MoveToAndFrom(double distanceFactor)
{
    assert(distanceFactor > 0.0);
    ScreenDistance *= distanceFactor;
    Position = Direction;
    Position *= -ScreenDistance;
    Position += ScreenCenter;
    ScreenHeight *= distanceFactor;
    ScreenWidth *= distanceFactor;
    pixeldU *= distanceFactor;
    pixeldV *= distanceFactor;
}


// Given a fully established camera and view, scale distance of viewer from
//		screen's  center. Rescales to keep same overall size
