...
void myRenderScene() {
    ...
    // Should really check if timeLocation and waveModeLocatioshadern are -1 before calling these,
...
void my_setup_SceneData() {
	...
	//GlShaderMgr::LoadShaderSource("PhongPhong.glsl");
	GlShaderMgr::LoadShaderSource("EduPhong.glsl");

    unsigned int shader_vert = GlShaderMgr::CompileShader("vertexShader_PhongPhong");
    unsigned int shader_frag = GlShaderMgr::CompileShader("fragmentShader_PhongPhong", "calcPhongLighting", "applyTextureMap");
	unsigned int shader_geom = GlShaderMgr::CompileShader("geomShader");
    unsigned int shaders_progs[3] = { shader_vert, shader_frag,  shader_geom};
